...
import { processBatch, type ProcessFileOptions } from '@/lib/agent/file-processor'
...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        console.log(`[Admin] Manual batch processing triggered by ${auth.user.email}`)
        ...
        if (fileIds && Array.isArray(fileIds) && fileIds.length > 0) {
            ...
            console.log(`[Admin] Processing ${filesToProcess.length} ${targetStatus} files`)
        }
