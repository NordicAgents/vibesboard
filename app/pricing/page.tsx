...
function formatPrice(plan: PlanDefinition): { price: string; suffix: string } {
  if (plan.pricePerSeat) {
    return {
      price: `$${(plan.pricePerSeat / 100).toFixed(0)}`,
      suffix: '/seat/month'
    }
  }
  if (plan.price === 0) {
...
function getPlanFeatures(plan: PlanDefinition): string[] {
  ...
  if (plan.includedMessagesPerSeat) {
    features.push(
      `${plan.includedMessagesPerSeat.toLocaleString()} messages/seat/month`
    )
  ...
  if (plan.featureFlags.includes('AGENT_NOTIFICATIONS'))
    features.push('Agent notifications')
  if (plan.featureFlags.includes('GOOGLE_REVIEW'))
    features.push('Google Review integration')
  if (plan.featureFlags.includes('INBOX')) features.push('Inbox')
  if (plan.featureFlags.includes('WHATSAPP_INBOX'))
    features.push('WhatsApp & Instagram Inbox')
  if (plan.featureFlags.includes('CHATWOOT'))
    features.push('Chatwoot integration')
  if (plan.featureFlags.includes('TEAM_COLLABORATION'))
    features.push('Team collaboration')
  if (plan.featureFlags.includes('CUSTOM_BRANDING'))
  ...
  const displayPlans = allPlans
    ...
    .map(plan => {
      ...
      return {
        ...
        ctaHref: '/sign-up',
        ctaAuthHref:
          plan.id === 'free' ? '/agents' : '/settings/tenant/billing',
      ...
      <div className="mx-auto max-w-5xl px-4 py-16 sm:px-6 sm:py-24">
        ...
        <div className="mb-12 text-center">
          ...
          <p className="mx-auto mt-4 max-w-xl text-lg text-[#6f7f80]">
            Start free with 100 messages. Upgrade when you need more. All plans
            include unlimited agents.
          </p>
        </div>
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {displayPlans.map(plan => (
            ...
            >
              ...
              <p className="mt-1 text-sm text-[#6f7f80]">{plan.description}</p>
