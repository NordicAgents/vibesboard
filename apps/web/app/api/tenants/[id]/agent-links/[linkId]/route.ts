...
import { getAgentLinkById, updateAgentLink, deleteAgentLink } from '@vibesboard/policy/agent-links/db'
...
export async function GET(req: Request, { params }: RouteParams) {
  ...
  if (!link) return NextResponse.json({ error: 'Agent link not found' }, { status: 404 })
...
export async function PATCH(req: Request, { params }: RouteParams) {
  ...
  if (!(await isFeatureEnabled(tenantId, 'AGENT_LINKS')))
    return NextResponse.json({ error: 'Agent Links feature is not enabled for this tenant' }, { status: 403 })

  const existing = await getAgentLinkById(tenantId, linkId)
  if (!existing) return NextResponse.json({ error: 'Agent link not found' }, { status: 404 })
  ...
  if (parsed.data.agentId !== undefined && !(await getAgentForMember(tenantId, parsed.data.agentId)))
...
export async function DELETE(req: Request, { params }: RouteParams) {
  ...
  if (!deleted) return NextResponse.json({ error: 'Agent link not found' }, { status: 404 })
