...
import { getAgentLinksForTenant, isLinkSlugAvailable, createAgentLink } from '@vibesboard/policy/agent-links/db'
...
export async function POST(req: Request, { params }: RouteParams) {
  ...
  if (!(await isLinkSlugAvailable(slug, tenantId)))
    return NextResponse.json({ error: 'This slug is already in use. Choose a different one.' }, { status: 409 })
  if (!(await getAgentForMember(tenantId, agentId)))
    return NextResponse.json({ error: 'Agent not found in this tenant' }, { status: 404 })

  const link = await createAgentLink({ tenantId, agentId, slug, name, description: description ?? null, createdBy: auth.user.id })
