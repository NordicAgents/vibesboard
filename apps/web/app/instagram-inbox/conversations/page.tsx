...
export default function InstagramInboxConversationsPage() {
  ...
  const handleAssignAgent = async (agentId: string) => {
    ...
    await fetch(
      `/api/tenants/${tenantId}/instagram-inbox/accounts/${selectedAccountId}`,
      {
        method: 'PATCH',
  ...
  const fetchConversations = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/tenants/${tenantId}/instagram-inbox/accounts/${selectedAccountId}/conversations${statusParam}`
      )
      if (res.ok) {
  ...
  const fetchMessages = useCallback(async () => {
    if (!tenantId || !selectedAccountId || !selectedContactId) return
    setLoadingMessages(true)
    try {
      const res = await fetch(
        `/api/tenants/${tenantId}/instagram-inbox/accounts/${selectedAccountId}/conversations/${selectedContactId}/messages?limit=100`
      )
      if (res.ok) {
  ...
  useEffect(() => {
    ...
    fetch(
      `/api/tenants/${tenantId}/instagram-inbox/accounts/${selectedAccountId}/conversations/${selectedContactId}`,
      {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  if (!loadingAccounts && accounts.length === 0) {
    return (
      <div className="container max-w-5xl py-6">
        <PageHeader title="Instagram Inbox" />
        <EmptyState
          icon={Instagram}
          title="No accounts connected"
          description="Connect an Instagram Business Account first to start receiving messages."
          action={
            <Button onClick={() => router.push('/instagram-inbox/accounts')}>
              Go to Accounts
            </Button>
          }
        />
