...
function GoogleCloudUrlBuilder({
  onApply
}: {
      ...
      {open && (
        <div className="space-y-2 border-t border-border px-3 pb-3 pt-2">
          <p className="text-muted-foreground">
            For Google Cloud MaaS models (e.g.{' '}
            <span className="font-mono">
              intfloat/multilingual-e5-large-instruct-maas
            </span>
            ). Use a service account key or{' '}
            <span className="font-mono">gcloud auth print-access-token</span> as
            the API key.
          </p>
...
const KIND_COLORS: Record<ProviderKind, string> = {
  openai:
    'bg-emerald-100 text-emerald-700 ring-1 ring-emerald-300 dark:bg-emerald-500/15 dark:text-emerald-300 dark:ring-emerald-500/40',
  anthropic:
    'bg-orange-100  text-orange-700  ring-1 ring-orange-300  dark:bg-orange-500/15  dark:text-orange-300  dark:ring-orange-500/40',
  openai_compatible:
    'bg-sky-100     text-sky-700     ring-1 ring-sky-300     dark:bg-sky-500/15     dark:text-sky-300     dark:ring-sky-500/40',
  google:
    'bg-purple-100  text-purple-700  ring-1 ring-purple-300  dark:bg-purple-500/15  dark:text-purple-300  dark:ring-purple-500/40',
  nvidia:
    'bg-lime-100    text-lime-700    ring-1 ring-lime-300    dark:bg-lime-500/15    dark:text-lime-300    dark:ring-lime-500/40'
}
...
const TASK_LABELS: Record<LlmTask, { label: string; description: string }> = {
  ...
  agent_creator: {
    label: 'Agent Builder',
    description: 'The AI that helps you create agents'
  },
...
export default function LlmProvidersPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (taskRes.ok)
        setTaskAssignments((await taskRes.json()).assignments ?? [])
      if (netRes.ok) {
  ...
  const handleKindChange = (kind: ProviderKind) => {
    ...
    setForm(f => ({
      ...f,
      kind,
      modelId: recommended?.id ?? DEFAULT_MODELS[kind]
    }))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = isEdit
        ? `/api/tenants/llm-configs/${(mode as { id: string }).id}`
        : '/api/tenants/llm-configs'
  ...
  const handleTest = async (id: string) => {
    setTesting(id)
    try {
      const res = await fetch(`/api/tenants/llm-configs/${id}/test`, {
        method: 'POST'
      })
  ...
  const saveNetworkSettings = async (patch: {
    llmAllowPrivateHosts?: boolean
    llmHostAllowlist?: string[]
  }) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error()
      if (patch.llmAllowPrivateHosts !== undefined)
        setAllowPrivateHosts(patch.llmAllowPrivateHosts)
      if (patch.llmHostAllowlist !== undefined)
        setHostAllowlist(patch.llmHostAllowlist)
  ...
  const removeAllowlistEntry = (host: string) => {
    saveNetworkSettings({
      llmHostAllowlist: hostAllowlist.filter(h => h !== host)
    })
  }
              ...
              <CardHeader className="pb-2">
                <div className="flex items-start justify-between gap-2">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span
                      className={`rounded-full px-2 py-0.5 text-xs font-medium ${KIND_COLORS[cfg.kind]}`}
                    >
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                      ...
                      <>
                        <select
                          ...
                          onChange={e => {
                            ...
                            setForm(f => ({
                              ...f,
                              modelId: value === '__custom__' ? '' : value
                            }))
                          }}
                          required
                        >
                ...
                {form.kind === 'nvidia' && (
                  <p className="text-xs text-muted-foreground">
                    ...
                    — the free tier gives rate-limited access to 100+ hosted
                    models (no credit card). Requests go to
                ...
                {form.kind === 'openai_compatible' && (
                  <div className="space-y-3">
                    ...
                    <div className="space-y-1 rounded-md border border-border bg-muted/50 px-3 py-2 text-xs text-muted-foreground">
                      ...
                      <p>
                        When assigning this provider to the{' '}
                        <span className="font-mono">Embeddings</span> task, use
                        one of these model IDs:
                      </p>
                      <ul className="mt-1 list-inside list-disc space-y-0.5">
                        <li>
                          ...
                          — 384-dim · Google Cloud AI Platform MaaS (use URL
                          builder above)
                        </li>
                        <li>
                          <button
                            ...
                            className="underline transition-colors hover:text-foreground"
                            onClick={() =>
                              setForm(f => ({ ...f, modelId: 'baai/bge-m3' }))
                            }
                          >
                            baai/bge-m3
                          </button>{' '}
                          — 1024-dim · NVIDIA free tier · multilingual · base
                        ...
                        <li>
                          <button
                            ...
                            onClick={() =>
                              setForm(f => ({
                                ...f,
                                modelId: 'snowflake/arctic-embed-l-v2.0'
                              }))
                            }
                          >
                            snowflake/arctic-embed-l-v2.0
                          </button>{' '}
                          — 1024-dim · NVIDIA free tier · English · base URL:
                        ...
                        <li>
                          ...
                          — 1024-dim · NVIDIA NIM ·{' '}
                          <span className="font-mono">input_type</span> auto-set
          ...
          <div className="mt-6 space-y-3">
            <div>
              ...
              <p className="mt-0.5 text-xs text-muted-foreground">
                Assign a specific provider to each task. Unassigned tasks use
            ...
            <div className="overflow-hidden rounded-lg border border-border">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-border">
                  {(['chat', 'embed', 'agent_creator', '*'] as LlmTask[]).map(
                    task => {
                      const assigned =
                        taskAssignments.find(a => a.task === task)?.configId ??
                        null
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-2">
                              <select
                                ...
                                onChange={e =>
                                  handleTaskAssign(task, e.target.value || null)
                                }
                              >
                                <option value="">
        ...
        {!isFormOpen && (
          <Card className="mt-2">
            <CardHeader className="pb-3">
              ...
              <CardDescription className="text-xs">
                Configure which hosts the server can reach for local or private
                LLM deployments (e.g. Ollama on your network).
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="text-sm font-medium">
                    Allow private / local hosts
                  </p>
                  <p className="mt-0.5 text-xs text-muted-foreground">
                    Permits base URLs with private IP ranges (10.x, 192.168.x,
                    localhost, etc.). Use for on-device or LAN-deployed models.
                  </p>
                </div>
                <button
                  type="button"
                  ...
                  onClick={() =>
                    saveNetworkSettings({
                      llmAllowPrivateHosts: !allowPrivateHosts
                    })
                  }
                ...
                >
                  <span
                    className={`pointer-events-none mt-0.5 inline-block size-5 rounded-full bg-white shadow-lg transition-transform ${allowPrivateHosts ? 'translate-x-5' : 'translate-x-0.5'}`}
                  />
                </button>
              </div>
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  Specific hostnames always permitted regardless of the toggle
                  above (e.g. <code className="text-xs">gpu-box.internal</code>,{' '}
                  <code className="text-xs">192.168.1.50</code>).
                </p>
                ...
                <div className="flex gap-2">
                  <Input
                    ...
                    onKeyDown={e =>
                      e.key === 'Enter' &&
                      (e.preventDefault(), addAllowlistEntry())
                    }
                  ...
                  <Button
                    ...
                    disabled={savingNetwork || !newAllowlistEntry.trim()}
                  >
                    Add
                  </Button>
                </div>
