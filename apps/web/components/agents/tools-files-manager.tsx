...
export function ToolsFilesManager({
  agent,
  onUpdate,
  canEdit,
  sourceUrls,
  onSourceUrlsChange
}: ToolsFilesManagerProps) {
  ...
  const handleFileUpload = useCallback(
    async (files: FileList | File[]) => {
      ...
      if (invalidFiles.length > 0) {
        toast.error(
          `Some files exceed 10MB limit: ${invalidFiles.map(f => f.name).join(', ')}`
        )
        return
      }
      ...
      try {
        const uploadPromises = fileArray.map(async (file, index) => {
          try {
            ...
            const urlRes = await fetch(
              `/api/agents/${agent.id}/files/upload-url`,
              {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
        ...
        const successfulUploads = results
          .filter(
            (r): r is PromiseFulfilledResult<{ path: string; file: File }> =>
              r.status === 'fulfilled'
          )
          .map(r => r.value)

        if (successfulUploads.length > 0) {
          const newFileKeys = Array.from(
            new Set([...fileKeys, ...successfulUploads.map(item => item.path)])
          )
          ...
          const ingested = ingestionResults.filter(
            r => r.status === 'fulfilled'
          ).length
          const failedIngest = ingestionResults.filter(
            r => r.status === 'rejected'
          ).length
          if (ingested > 0) {
            toast.success(
              `Indexed ${ingested} file${ingested > 1 ? 's' : ''} for search`
            )
          }
          if (failedIngest > 0) {
            toast.error(
              `${failedIngest} file${failedIngest > 1 ? 's' : ''} failed to index`
            )
          }
          setIsIndexing(false)
        }
  ...
  const handleFileDownload = async (path: string) => {
    try {
      ...
      const res = await fetch(
        `/api/agents/${agent.id}/files/download-url?fileKey=${encodeURIComponent(path)}`
      )
      if (!res.ok) {
      ...
      <CardContent className="space-y-6">
        {!canEdit && (
          <p className="text-xs text-muted-foreground">
            Read-only. Ask a tenant admin (or the agent owner) to update tools
            and files.
          </p>
        )}
        {/* Tools Section */}
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Control whether the agent can reach the web or search uploaded
                files.
              </p>
            </div>
            <Button
          ...
          <div className="space-y-2">
            <div className="space-y-2 rounded-md border bg-muted/50">
              <div className="flex items-center justify-between p-3">
                <div className="pr-4">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Let the agent fetch content from provided URLs when it needs
                    extra context.
                  </p>
                </div>
                <Switch
                  ...
                  <div className="space-y-1.5">
                    ...
                    {sourceUrls.length === 0 && (
                      <p className="py-1 text-center text-xs text-muted-foreground">
                        No source URLs. Add URLs to auto-load content into agent
                        context.
                      </p>
                    )}
                  </div>
            ...
            <div className="flex items-center justify-between rounded-md border bg-muted/50 p-3">
              <div className="pr-4">
                ...
                <p className="text-xs text-muted-foreground">
                  Allow the agent to ground responses in the files you upload
                  here.
                </p>
              </div>
              <Switch
        ...
        <div className="space-y-3">
          ...
          {fileKeys.length > 0 ? (
            ...
            <p className="py-4 text-center text-xs text-muted-foreground">
              No files uploaded yet. Upload documents to help your agent provide
              better responses.
            </p>
          )}
        </div>
