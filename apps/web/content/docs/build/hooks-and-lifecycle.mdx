...
The `202` body is `jobId`, `status: "pending"`, `agentId`, and `hookId` — no reply yet. The workspace usage limit is checked when the job runs rather than at submission, so an over-limit async call is accepted and then fails with a `Usage limit reached` error. Poll it at any time with the same secret:
...
<Callout type="note" title="A rejected callback URL surfaces as a failed job">
