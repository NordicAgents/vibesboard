...
workspace packages under `packages/*`. The workspace globs are `apps/web`,
`packages/*` and `ee/*`, declared in the root `package.json`. Everything under
`ee/` is the Enterprise Edition and is **not** MIT licensed — see
[Open core & ee/](/docs/contribute/open-core). It is optional: the community
edition builds with that directory deleted. This page is the map —
