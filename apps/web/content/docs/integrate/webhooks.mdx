...
`data` is whatever the model passed to the tool, rewritten by the agent's field mappings (collection-field ID or column label, matched case-insensitively) if any are configured — see [Data actions & tools](/docs/build/data-actions-and-tools#submit_data--always-available) for how mapping works.
