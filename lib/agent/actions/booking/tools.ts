...
function buildCheckAvailabilityTool(agent: VibeAgent, config: BookingConfig): RegisteredTool {
  ...
  return {
    ...
    execute: async (args) => {
      ...
      try {
        const busySlots = await checkFreeBusy(accessToken, calendarId, new Date(now), windowEnd)
        ...
        const slots = suggestions
          .map((s, i) => `${i + 1}. ${formatDateRange(s, durationMs, timezone)}`)
          .join('\n')
...
function buildCreateBookingTool(agent: VibeAgent, config: BookingConfig): RegisteredTool {
  ...
  const titleTemplate = config.eventTitleTemplate ?? '{guest_name} ({guest_count} guests)'
    ...
    function: {
      ...
      parameters: {
        ...
        properties: isEnquiry
          ...
          : {
              resource_name: { type: 'string', description: `Resource name. One of: ${resourceNames}.` },
  ...
  return {
    ...
    execute: async (args) => {
      ...
      try {
        ...
        const updated = await updateCalEvent(accessToken, resource.calendarId, eventId, updates)
