...
async function fetchBusySlots(
  ...
  const raw: Array<{ start: string; end: string }> =
...
async function queryAvailability(
  ...
  const slots = suggestions
    .map((s, i) => `${i + 1}. ${formatSlot(s, durationMs, ctx.timezone)}`)
    .join('\n')
...
function buildCheckAvailabilityTool(agent: VibeAgent): RegisteredTool {
  const resourceNames = agent
    .bookingConfig!.resources.map(r => r.name)
    .join(', ')
...
export function buildSimpleBookingTools(agent: VibeAgent): RegisteredTool[] {
  ...
  return [buildCheckAvailabilityTool(agent), buildSubmitEnquiryTool(agent)]
}
