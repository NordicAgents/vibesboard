...
import { stripCompletionMarkers, detectCompletionMarker } from "@/lib/agent/completion";
import {
...
export async function handleWhatsAppMessage(
  ...
  console.log(`📨 Processing message from ${phoneNumber} for agent ${agent.name}`);
  ...
  try {
    ...
    const maxMessagesReached =
      agent.max_messages && userMessageCount >= agent.max_messages;

    const shouldComplete = isComplete || maxMessagesReached;

    console.log(`🎯 Completion status: ${shouldComplete ? "COMPLETE" : "ongoing"}`);
    if (completionReason) {
      console.log(`   Reason: ${completionReason}`);
    }
    if (maxMessagesReached) {
      console.log(`   Max messages reached: ${userMessageCount}/${agent.max_messages}`);
    }
